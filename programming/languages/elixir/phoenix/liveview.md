...
* Implement `on_mount/4`

```elixir
defmodule MyApp.MyHookModule do
  use Phoenix.LiveView

  def on_mount(_type, _params, _session, socket) do
    {:ok, socket}
  end
end
```

`_type` defaults to `:default`.

To activate on all live views:
...
```elixir
  def live_view do
    quote do
      ...
      on_mount MyApp.MyHookModule
    end
  end
```

## For Other Life-cycle Stages
